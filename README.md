...
Le [rapport d'analyse](https://github.com/cvarrei/immo_TD/blob/main/Rapport%20d'analyse/rapport_analyse_final.pdf) est un document pdf expliquant de manière textuelle et simplifiée le processus de préprocessing, puis d'analyse univariée et bivariée des variables présentes dans le dataset originelainsi que des variables open-data ajoutées. Il est complétée par une description rapide des variables importantes pour les différents modèles de prédiciton. Il est accompagné de sa version en JupyterNotebook ainsi que des plots en format png pour le notebook.
